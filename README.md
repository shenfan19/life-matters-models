...
> LMML（Life Matters Model Language）格式的 YAML 模型库。  
...
每个模型是一个 YAML 文件，按 LMML 格式编写，可由 LM 仿真引擎直接运行和优化。

LMML 是一种开放的 YAML 格式标准，类似 SBML / CellML，但专注于：
- **个体尺度**的健康与行为动力学（分钟～年）
- **行为干预调度**（饮食、运动、用药时序）
...
| 文档 | 内容 |
|------|------|
| [docs/LMML_1.0.md](docs/LMML_1.0.md) | LMML 格式规范全文（学术版） |
