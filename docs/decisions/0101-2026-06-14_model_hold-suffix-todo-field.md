...
ADR 0096 用 `_nosim` / `_noopt` / `_noref` 三种文件名后缀分别标记 sim、optimizer、文献来源三个维度的"未完成"状态，组合写法（如 `_nosim_noopt_noref`）在实践中暴露两个问题：

1. **后缀不携带诊断信息**：一个 `_noopt` 文件只说明"optimizer 失败"，不说明*为什么*。每次复查（无论 AI 还是人工）都要重新跑一遍 `--opt`、重新分析日志和 Pareto 前沿，诊断过程不可复用。
