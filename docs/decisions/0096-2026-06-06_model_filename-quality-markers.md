...
**日期**：2026-06-06  
**状态**：⚪ 已被 [0101](0101-2026-06-14_model_hold-suffix-todo-field.md) 取代（`_nosim`/`_noopt`/`_noref` 三后缀已于 2026-06-14 全量迁移为 `_HOLD` + `metadata.todo`）  
**类别**：模型库管理 / 工程约定  
...
2. **质量状态不可见**：sim 是否通过、optimization 是否通过、文献来源是否完整，没有统一的标记方式。之前使用 `_noref`（缺文献）和 `_mw`（纯组件）两种后缀，约定不完整、语义不对称。
...
| `_noopt` | sim 通过，optimization 失败（算法报错、约束违反等） | ✓ |
