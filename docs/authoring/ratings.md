...
metadata:
  ...
  ratings:
    ...
    validation_confidence: 3 - 说明 # 通用字段（所有类型，跑仿真验证后才能打分）
...
以下字段定义跨类型一致，适用于 papers/、scenarios/、references/ 中的任何模型。
...
### `validation_confidence` — 仿真验证置信度（1-5 量表，2026-07-22 新增）

可选字段，与 `topic_importance`/`evidence_quality` 等同级。

```yaml
metadata:
  ratings:
    validation_confidence: 4 - 一句话说明依据（如"BP降幅分解后落在文献区间，UA升幅因场景混杂未达标"）
```

只写一个 1-5 的数字加一句话理由，不展开成子字段，更新时间跟随该文件本身的 `metadata.updated`，不单独维护日期。评的是这个模型的仿真输出实测有没有对上独立文献目标，跟 `evidence_quality` 即引用的文献本身质量高不高、作者主观判断的一次性打分，是两个不同的轴，不要混用。

量表定义：

| 分数 | 含义 |
| --- | --- |
| 1 | 尚未验证：仅有静态参数声明，未跑通 `--sim`/`--opt`，或已知存在阻断性 bug |
| 2 | 跑通但未对上文献：能正常输出仿真轨迹，但未与独立文献目标比较，或比较后方向性不符（系数疑似不同源、单位/时间尺度换算未被追踪等） |
| 3 | 部分对标：与文献目标部分吻合，或吻合但场景/样本存在明显局限（混杂因素未剥离、单一样本代表性有限等） |
| 4 | 基本吻合：核心验证点落在文献目标范围内，偏差在可接受量级；或所声明的 evidence 效应量本身来自高质量独立来源、Loader 正确编码展示（非独立预测时需在理由文本注明） |
| 5 | 独立预测检验通过：用不参与拟合的独立数据点做留一法预测，结果与文献报告吻合，或多个独立来源交叉印证一致 |

跨学科验证案例汇总与逐模型详细依据见 `models/test_validation/validation_report.md`；论文引用模型时，优先选 `validation_confidence >= 4` 的模型作为核心案例，3 分及以下的模型适合用于展示方法论/框架能力，但需要在论文正文里说明局限，不宜作为强定量结论的支撑。
...
> **建议补充字段**（可选）：
> - `reviewer_robustness`: 核心结论对参数估算质疑的稳健程度（5=核心结论不依赖任何存在争议的估算参数，1=核心结论高度依赖某个有争议的估算参数）
...
- 模型在论文中的角色定位变化（如晋升为旗舰案例或降为附录）后，同步更新 `paper_value`。
